			   ...
			   vir_bytes buf, size_t size, endpoint_t for_e)
{
	...
	// -----------------------------------------------------------------
	struct vmnt *vpath;
	vpath = find_vmnt(vp->v_fs_e); // get the path object of the file using file system endpoint
	if(strcmp(vpath->m_mount_path, "/home") == 0) // check if mount path is /home
	{
		if (rw_flag == READING)  // depending on flag, print.
		{
			printf("Minix (R.No.:03) file read: %llu; nbytes = %zu; offset = %llu\n", vp->v_inode_nr, size, f->filp_pos);
		} 
		if (rw_flag == WRITING)
		{
			printf("Minix (R.No.:03) file write: %llu; nbytes = %zu; offset = %llu\n", vp->v_inode_nr, size, f->filp_pos);
		}
	}
	// -----------------------------------------------------------------
