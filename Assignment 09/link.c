...
int do_unlink(void)
{
  ...
  // -----------------------------------------------------------------
  // based on code from above
  lookup_init(&stickycheck, resolve.l_path, PATH_RET_SYMLINK, &vmp2, &vp); // lookup inode of file
  ...
  if (strcmp(vmp->m_mount_path, "/home") == 0) // if mount path is /home then print 
  {
    printf("Minix (R.No.:03) file deleted: %llu\n", vp->v_inode_nr);
  }

  if (vp != NULL) // unlock + freeing
  {
    unlock_vnode(vp);
    put_vnode(vp);
  }
