...
int common_open(char path[PATH_MAX], int oflags, mode_t omode)
{
	...
	if (oflags & O_CREAT)
	{
		...
		r = err_code;
		if (r == OK) {
			exist = FALSE; /* We just created the file */ 		// <----- this comment helped !

			// -------------------------------------
			
			struct vmnt *vpath;
			vpath = find_vmnt(vp->v_fs_e); // gets the vmnt (virtual mount point) of the file, using its file sys endpoint
			if(strcmp(vpath->m_mount_path, "/home") == 0) 	// we need to print only if mount path is / 
				printf("Minix (R.No.:03) file created: %llu\n", vp->v_inode_nr);
